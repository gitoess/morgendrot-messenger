/**
 * Einsatz-Rollen-Templates — gleiche Regeln wie `src/einsatz-role-templates.ts` (`parseEinsatzRoleTemplates`).
 * Bei Regeländerungen dort **und hier** anpassen (oder später nach `src/shared` extrahieren).
 */
import type { EinsatzRoleTemplate } from '@/frontend/lib/api/einsatz-role-templates'
...
/** Wie Server-`parseEinsatzRoleTemplates`: `raw` = `{ templates: [...] }`. */
export function validateEinsatzRoleTemplatesBody(
  raw: unknown
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (!id || !isId(id)) {
      return {
        ok: false,
        error: `templates[${i}].id: gültige ID (Kleinbuchstaben, Zahlen, Bindestrich) erforderlich.`,
      }
    }
  ...
  if (new TextEncoder().encode(ser).length > EINSATZ_TEMPLATES_FILE_MAX_BYTES) {
    return {
      ok: false,
      error: `Gesamtdatei zu groß (max. ${EINSATZ_TEMPLATES_FILE_MAX_BYTES} Bytes).`,
    }
  }
