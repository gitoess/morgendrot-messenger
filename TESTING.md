...
**Merge-Ritual (Phase A, vor größeren PRs):** unten § **Qualitätsritual vor Merge** — deckt sich mit **`docs/PHASE-A-QUALITY-BASELINE-AND-TESTS.md`** Phase 1 und spiegelt **`.github/workflows/frontend-checks.yml`** (Frontend) plus Root-Smoke. **Messenger-Modularität (**§ H.1b**):** Schritt-für-Schritt-Ablauf **`docs/MESSENGER-UI-MODULARITY-STRATEGY.md`** § **5**. **Pull Requests:** Checkliste in **`.github/pull_request_template.md`**.

**Wann Handy / dokumentierte Läufe:** **`docs/HANDY-TEST-WINDOW.md`** — **`docs/TEST-RUN-LOGBOOK.md`** (Smoke, Frontend, Core, Realworld-Status).

**Protokoll (Architektur → Code, schrittweise):** Cold-Start / Geräte-Uhr — **`src/shared/device-time-trust.ts`** + Root-Test **`scripts/run-tests.ts`**; Spiegel **`frontend/frontend/lib/device-time-trust.ts`** + Vitest **`device-time-trust.test.ts`**; **`fetchStatus`** → **`pollClockHint`**; **`use-chat-view-api-status-poll`**: privater Chat → einmaliger **`navigator.geolocation.getCurrentPosition`** für **`hasTrustedGpsUtcFix`** (Nutzerdialog); Chat-Header **Warn-Banner**. Spec **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** §6; Fahrplan **§ H.6c**, **§ H.6e** (`.env` vs. Runtime). **Android (nativ, Zielbild):** Foreground Service + ehrliche Notification — **Doku** **`docs/ANDROID-FOREGROUND-SERVICE-MINIMAL-SYNC.md`**, Fahrplan **§ H.6f**; nach Einführung einer nativen Hülle: manuell auf Referenzgerät prüfen (Notification-Kanal, Akku-Stopp, SOS); **kein** Ersatz für **`npm run test:smoke`**. **Mailbox-Offline-Outbox (Entwicklung):** Opt-in **`localStorage.setItem('morgendrot.offlineMailboxQueue','1')`** — **`frontend/frontend/lib/api/offline-queue.ts`**, **`docs/MESSENGER-CHAT-INBOX-ARCHITEKTUR.md`** §5; **`canonical_msg_ref`** (§ **H.12**, vgl. **`docs/LORA-IOTA-DELAYED-UPLOAD-SPEC.md`**) wird im Core berechnet und persistiert. *Nächster Schritt: Attestation-Queue mit „Zeit unsicher“ / Sparse-Luma.*
...
3. **Optional (Chain / IOTA-CLI) — zwei verschiedene Realworld-Pfade:**  
   - **Messenger** (Chat, Mailbox, …): **`npm run test:messages`** / **`npm run test:messenger`** / **`npm run test:messages:single`** → **`scripts/run-messages-chat-realworld.ts`** (API(s) laufen, Tresor entsperrt — in der **UI** reicht; **`UNLOCK_PASSWORD_*`** in der Env ist **optional** und nur sinnvoll, wenn das Skript **`POST /api/unlock`** selbst ausführen soll. Abschnitt **`/vault-save`**: ohne Env ruft das Skript den Befehl mit leeren Args auf, der **Server** nutzt dann das Passwort aus dem UI-Unlock (`getWalletPassword()`); siehe Skriptkopf.  
   - **Tickets & AccessKey** (Event-/Schloss-Kacheln, nicht der Messenger-Chat): **`npm run test:realworld`** oder explizit **`npm run test:tickets-accesskey-realworld`** → **`scripts/run-ticket-accesskey-realworld.ts`**.  
   Wenn die **IOTA-CLI** und der **RPC-Server** unterschiedliche **API-Versionen** melden, schlagen Ticket/Key-Schritte fehl („**Client/Server api version mismatch**“) — typisch ab dem Schritt **„Personalisiertes Ticket“** (zusätzliche CLI-PTB). **IOTA-CLI** (`iota --version` o. ä.) an die **RPC_URL**-Node-Version angleichen, Skript erneut. Lauflog: **`docs/TEST-RUN-LOGBOOK.md`**. Ersetzt **nicht** die Chain-freien Smoke-Tests oben.
