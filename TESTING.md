...
3. **Optional (Chain / IOTA-CLI):** **`npm run test:realworld`** — Ticket- und AccessKey-Szenarien gegen die konfigurierte RPC-Umgebung. Wenn die **IOTA-CLI** und der **RPC-Server** unterschiedliche **API-Versionen** melden, schlagen einzelne Schritte fehl („Client/Server api version mismatch“); CLI aktualisieren bzw. RPC-Endpoint an die installierte CLI anpassen, dann erneut ausführen. Ersetzt **nicht** die Chain-freien Smoke-Tests oben.
