...
**Wann Handy / dokumentierte Läufe:** **`docs/HANDY-TEST-WINDOW.md`** — **`docs/TEST-RUN-LOGBOOK.md`** (Smoke, Frontend, Core; Messenger- und Ticket-Läufe **getrennt**).
...
| 5c | **`npm run test:h15-direct-submit`** | **`direct-iota-plain-submit.test.ts`** (§ **H.15** Direkt vs. Relay / Frühabbrüche); bei Touch von **`mailbox-send-hybrid`**, **`execute-command`**, **`chat-commands`**, **`messaging-persistence-mode`**, **`use-chat-view-handle-send`** zusätzlich **`npm run test:unit`** (u. a. **`execute-command.test.ts`**) — Dateien: **`direct-iota-plain-submit.ts`**, **`direct-iota-encrypted-submit.ts`**, **`mailbox-send-hybrid.ts`**, **`execute-command.ts`**, **`chat-commands.ts`**, **`messaging-persistence-mode.ts`**, **`chat-view-transport-card.tsx`**, **`chat-view-pulse-settings.tsx`**, **`offline-queue.ts`**, **`use-chat-view-handle-send.ts`**, **`use-chat-view-mirror-delay.ts`**, **`use-chat-view-confirm-lora-online.ts`**, **`einsatzprotokoll-anchor.ts`**, **`chat-view-chat-header.tsx`** |
...
3. **Optional (Chain / IOTA-CLI) — zwei getrennte Realworld-Pfade (nicht vermischen):**  
