...
3. **Mit Next:** `npm run dev` → **http://127.0.0.1:3341/** — Chat-Kachel: Credits-Balken erscheint, wenn **`MESSENGER_CREDITS_OBJECT_ID`** gültig ist und die API das Objekt lesen kann; Lock-Kachel: Statuszeile „Backend / Chat / Keys“. Optional: **`/handbook`** (Markdown-Handbuch; Produktion: SW-Cache) — siehe **`docs/PWA-HANDBUCH-OFFLINE.md`**.
