...
|--------|----------------|--------------------|----------------------------------------|---------------------------|
| **1. Cold-Start / Zeit** | Ohne Internet: **unsichere Systemzeit** → IOTA-/Log-Zeitstempel und „letzte Teamposition“ sind **interpretierbar falsch**. | **Zeit vor erster signierter Nachricht** plausibilisieren: Online → **explizite** Referenzzeit (Server-Header, Indexer/Chain im Toleranzfenster — **kein** blindes Vertrauen in `Date` allein); **Offline** → **GPS-UTC** (Fix vom **Handy** oder Modul am **T-Beam** o. ä.), sonst **UI: „Uhr unsicher — Attestation/Chain zurückstellen“**; relative Reihenfolge intern trotzdem führen (**Monotonic-ID**). | **Schritt 1–2 (Ist):** Shared-Modul + **`fetchStatus` → `pollClockHint`** + Messenger-**Banner** (`ChatViewChatHeader`); Vitest **`device-time-trust.test.ts`**; **Nächster Schritt:** `hasTrustedGpsUtcFix` (Geolocation) + Attestation-**Gate**; Spec **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** §6. | Heltec **ohne** GPS: liefert **keine** autonome UTC — **kein** Wunder erwarten; Hardware-Mix in **`docs/LORA-EU-FUNK-HARDWARE-EINSATZPROFILE.md`**. |
