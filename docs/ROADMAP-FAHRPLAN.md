...
|--------|----------------|--------------------|----------------------------------------|---------------------------|
| **1. Cold-Start / Zeit** | Ohne Internet: **unsichere Systemzeit** → IOTA-/Log-Zeitstempel und „letzte Teamposition“ sind **interpretierbar falsch**. | **Zeit vor erster signierter Nachricht** plausibilisieren: Online → NTP/Chain-Hint; **Offline** → **GPS-Zeit** (Fix vom **Handy** oder Modul am **T-Beam** o. ä.), sonst **UI: „Uhr unsicher — Attestation/Chain zurückstellen“**; relative Reihenfolge intern trotzdem führen (**Monotonic-ID**). | PWA/Next + später **RN**: Dialog/Status, Blockierung oder Warnstufe für **MSG_ATTESTATION** / exportierte Logs bei `timeUncertainty`; Spez-Ergänzung in **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** / Betriebshandbuch. | Heltec **ohne** GPS: liefert **keine** autonome UTC — **kein** Wunder erwarten; Hardware-Mix in **`docs/LORA-EU-FUNK-HARDWARE-EINSATZPROFILE.md`**. |
...
|--------------|----------------------------|------------|
| **Jetzt bis ~8 Wochen (parallel Phase A)** | **`@morgendrot/core`**: bestehende **`src/shared`**-Module **schrittweise** extrahieren/verpacken; **Zeit-Unsicherheit** (H.6c) in Queue/Attestation-Fluss **modellieren**; **Teilbild-/Sparse-UI** wo LUMA/MF1 schon existieren. | **§ H.4**-Ritual; **kein** Blockieren von **§ H.2** / kleinen **§ H.1b**-Scheiben. |
