...
**Aktuell sinnvolle „nächste drei“ konkrete Arbeiten (Stand Doku 2026-03):** (1) **§ H.2** Punkt 1 — **`docs/PWA-MANUAL-CHECKS.md`** abarbeiten und Lücken notieren; (2) **§ H.1b** *Als Nächstes* — ein weiterer kleiner Vitest-Slice oder API-Barrel angleichen, **ohne** Mesh-Dateien anzufassen; (3) **§ H.3g** Pakete **1–2–6** prüfen ob noch UI-Lücken, bevor **7** oder Phase B Queue anfängt. **Phase B:** Spec **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** (**§ H.3n**) vor Implementierung mit **`emergency-binary-wire.ts`** und **`LORA-IOTA-DELAYED-UPLOAD-SPEC`** abgleichen.
...
- **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** – Offline/Online, Idempotenz, CRDT-Grenzen, Mehrgeräte; **§ H.12**; **§6** Geräte-Uhr (Cold-Start) + **`src/shared/device-time-trust.ts`**
...
|--------|----------------|--------------------|----------------------------------------|---------------------------|
| **1. Cold-Start / Zeit** | Ohne Internet: **unsichere Systemzeit** → IOTA-/Log-Zeitstempel und „letzte Teamposition“ sind **interpretierbar falsch**. | **Zeit vor erster signierter Nachricht** plausibilisieren: Online → **explizite** Referenzzeit (Server-Header, Indexer/Chain im Toleranzfenster — **kein** blindes Vertrauen in `Date` allein); **Offline** → **GPS-UTC** (Fix vom **Handy** oder Modul am **T-Beam** o. ä.), sonst **UI: „Uhr unsicher — Attestation/Chain zurückstellen“**; relative Reihenfolge intern trotzdem führen (**Monotonic-ID**). | **Schritt 1 (Ist):** `inferDeviceTimeTrust` in **`src/shared/device-time-trust.ts`** + Tests in **`scripts/run-tests.ts`**; PWA/Next + später **RN**: Signale aus `navigator` / API / GPS füttern, Warnstufe für **MSG_ATTESTATION** / Export; Spec **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** §6. | Heltec **ohne** GPS: liefert **keine** autonome UTC — **kein** Wunder erwarten; Hardware-Mix in **`docs/LORA-EU-FUNK-HARDWARE-EINSATZPROFILE.md`**. |
| **2. Teil-Nachrichten / Fragmentation** | Von **Luma** o. ä. kommen nur **Teile** an (Bewegung, Akku, Störung). | **„Halbes Bild besser als keins“**: Sparse-/Progressive-Anzeige aus **bekannten Chunks**; klar kennzeichnen **„unvollständig“**. | **`@morgendrot/core` + UI:** MF1-/Chunk-Metadaten nutzen; **Decoder-Pfad** für Vorschau aus partiellen LUMA/CHROMA-Daten (auch **niedrig aufgelöst**); einheitliche Policy in Attestation-Spec. **Kein** Warten auf 100 % für **Rettungs-Anzeige**. | Funk bleibt **Paketweise**; Robustheit = **App-Reassembly + Anzeige**, nicht nur Retry. |
| **3. Physische Sicherung (verlorenes Heltec)** | Gerät im Wald: **Puffer** kann **fremde** oder **eigene** Chunks enthalten; Auslesen des Flash **ohne** Auth. | **Defense in depth:** Was auf dem Funkgerät liegt, sollte **für Fremde nutzlos** sein (Krypto), **Metadaten** trotzdem minimieren. | **Minimale Retention** (kurz puffern, nach erfolgreichem **Pull**/`ACK` löschen wo API das hergibt), **Pairing/Bonding**-Disziplin; **Inhalt** weiter **E2E** (bereits Richtung). | **„LittleFS-Vollverschlüsselung“** auf Stock-Meshtastic als **Morgendrot-Feature** = **eigenes Firmware-/Sidechannel-Thema** — **nicht** vor klarer Threat-Model-Spec; Backlog **Phase C** oder **eigener** Transport (nicht parallel zum Mesh-MVP erzwingen). |
...
| **RN + Expo** | **Neue Laufzeit** neben PWA: **sinnvoll**, wenn **Core-Schnitt** oder **klare API-Grenze** steht — **nicht** in derselben Woche wie großer **§ H.1b**- oder Mesh-Kern-Wechsel (**§ C.0b**). |
...
|--------------|----------------------------|------------|
| **Jetzt bis ~8 Wochen (parallel Phase A)** | **`@morgendrot/core`**: bestehende **`src/shared`**-Module **schrittweise** extrahieren/verpacken; **Zeit-Unsicherheit** (H.6c): **Schritt 1 erledigt** — `inferDeviceTimeTrust` in **`src/shared/device-time-trust.ts`** + Root-Test; **nächster Schritt:** Signale in PWA/RN füttern, UI-Warnung vor Attestation/Export; **Teilbild-/Sparse-UI** wo LUMA/MF1 schon existieren. | **§ H.4**-Ritual; **kein** Blockieren von **§ H.2** / kleinen **§ H.1b**-Scheiben. |
...
| **React Native + Expo** | **Port** der stabilen Core-API + **native** BLE/STT/TTS — **eigener** Meilenstein. | **Entweder** nach erstem **publishierbaren** `core`-Paket **oder** mit **explizit** abgeteilter Person/Woche (sonst reißt **§ C.0b**). |
