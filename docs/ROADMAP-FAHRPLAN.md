...
|--------------|----------------------------|------------|
| **Jetzt bis ~8 Wochen (parallel Phase A)** | **`@morgendrot/core`**: **`src/shared`** extrahieren; **Zeit** (H.6c): Modul + `pollClockHint` + Banner + **GPS-Probe** (privater Chat); **Teilbild-/Sparse-Luma** als Quick-Win; **Konfiguration** (H.6e): Core-**Konstanten** + Runtime-**Storage**, **keine** schwere `.env`-Abhängigkeit im Core-Paket. | **§ H.4**-Ritual; **§ H.2** / **§ H.1b** nicht blockieren. |
