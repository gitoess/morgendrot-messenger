...
**Aktuell sinnvolle „nächste drei“ konkrete Arbeiten (Stand Doku 2026-03-30):** (1) **§ H.2** — Vorprüfung A–C in **`docs/PWA-MANUAL-CHECKS.md`** (Protokoll + Tabelle **Lücken / nur am Gerät**); Feld-Checks **#1–5** weiter manuell. (2) **§ H.1b** — Dashboard-Imports auf **`@/frontend/lib/api`** (**erledigt**); optional **`frontend/lib/api.ts`-Re-Export** später entfernen, wenn keine Altimporte mehr. (3) **§ H.3g** Pakete **1–2–6**: Next-PWA **Einsatz-Rollen-Vorlagen** (Einstellungen) + **Handbuch-/SW-Offline** für **`API-EINSATZ-ROLE-TEMPLATES.md`**; **Paket 7** (Offline-Relay-Queue) erst mit **§ H.12** / Phase B. **Kürzlich umgesetzt / eingeordnet:** **§ H.6c** zusätzliche Bestätigung vor **Einsatz-Exporten**, wenn **`deviceTimeTrustWarn`**; **Sparse-Luma**-Sende-Fortschritt (**Mesh v2** LUMA/CHROMA-Paketzähler in der Anhang-Leiste). **Phase B:** Spec **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** (**§ H.3n**) vor größeren Eingriffen mit **`emergency-binary-wire.ts`** und **`LORA-IOTA-DELAYED-UPLOAD-SPEC`** abgleichen.
...
**Als Nächstes (klein, empfohlen):** (a) **`frontend/lib/api.ts`-Re-Export** entfernen, sobald keine externen Altimporte mehr nötig sind; (b) § **H.1a** weiterer RTL-/Vitest-Slice am **Send-Panel** / Inbox-Rand, wenn Mesh-Kern nicht parallel stark bewegt wird; (c) weitere ESLint-Zonen nur ergänzen, wenn **messbare** neue Querimports auftauchen (z. B. **send**↔**attachments**).
