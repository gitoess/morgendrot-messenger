...
**Aktuell sinnvolle „nächste drei“ konkrete Arbeiten (Stand Doku 2026-04-28):** (1) **§ H.15** — Stufe **2** auf Testnet **einmal** nach **`docs/HANDY-FIRST-STAGE2-CLIENT-SUBMIT-SMOKE.md`** verifizieren (Digest/Fehlerpfad); Stufe **4** Ritual = **`npm run test:h15-direct-submit`** + Zeile **5c** in **`TESTING.md`** (Ist). (2) **§ H.15** — **Stufe 3** weiter verfeinern (**`messageNonceU64`**-Durchreichung, Konflikt-UI) unter **§ H.12** — Backoff/Defer dokumentiert in **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** § 8.1. (3) **§ H.1b** — nächste **kleine** Scheibe (**`lib/`**-Helfer oder ESLint-Zonen) — **nicht** parallel großer Mesh-Kern. **PWA-Schreibtisch (§ C.0b Stufe 0 / § H.2):** Protokollzeile **2026-04-28** in **`docs/PWA-MANUAL-CHECKS.md`** — nach Deploy **`check:pwa-desk`** / bei Release **`check:pwa-desk:full`**; **L1–L5** am Gerät. **Kürzlich dokumentiert:** **2026-04-28** neue Primärleitlinie **`docs/BACKEND-VS-DIREKT-IOTA-ERKLAERUNG.md`** § 6 + **`docs/ARCHITECTURE-HANDY-FIRST-CLIENT-IOTA.md`** + **§ H.15**. **Paket 7 voll** (Offline-Relay / Settlement) erst mit **§ H.12** / Phase B — **Paket 7a** bleibt **Vorbereitung**. **Phase B:** Spec **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** (**§ H.3n**) vor größeren Eingriffen mit **`emergency-binary-wire.ts`** und **`LORA-IOTA-DELAYED-UPLOAD-SPEC`** abgleichen.
