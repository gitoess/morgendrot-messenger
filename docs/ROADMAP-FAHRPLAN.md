...
**Aktuell sinnvolle „nächste drei“ konkrete Arbeiten (Stand Doku 2026-04-16):** (1) **§ H.2** — Manuelle **L1–L5** am Gerät/Browser laut **`docs/PWA-MANUAL-CHECKS.md`**, wenn Deploy oder Service-Worker-Wechsel ansteht; automatisiert **A+B+C** zuletzt mit **`npm run check:pwa-desk:full`** (2026-04-16). (2) **§ H.0** — UX-Lücken nach Priorität (Handy/Entsperren, schlanke Oberfläche, siehe Fahrplan **§ H.0**). (3) **§ H.3g** — Pakete **1→2→6** (Einsatz-Rollen-Vorlagen, Handbuch/SW-Offline) **oder** nächste **§ H.1b**-Scheibe aus **`docs/MESSENGER-UI-MODULARITY-STRATEGY.md`** § 5 (kleine Schritte, nicht mit Phase-B-Mesh kreuzen). **Kürzlich umgesetzt / eingeordnet:** **§ H.1b** `pickInboxRawMessages` → **`lib/inbox-pick-raw-messages.ts`** + Vitest, ESLint **`.next`**-Ignore; **§ H.2** **`check:pwa-desk:full`** + Protokollzeile; **`next-env.d.ts`** Routes-Pfad nach Production-Build. **Paket 7 voll** (Offline-Relay / Settlement) erst mit **§ H.12** / Phase B — **Paket 7a** bleibt **Vorbereitung**. **Phase B:** Spec **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** (**§ H.3n**) vor größeren Eingriffen mit **`emergency-binary-wire.ts`** und **`LORA-IOTA-DELAYED-UPLOAD-SPEC`** abgleichen.
