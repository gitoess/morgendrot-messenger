...
| **Ist-Code (Messenger)** | **B1+B2:** UI-SOS, `MORG_EMERGENCY_V1` (**§7** Freeze), Mesh-Flash-Burst, **Backoff-Retry** (`morg-sos-mesh-retry`), **IOTA-Spiegel** nach erfolgreichem Funk-SOS (Opt-out `localStorage`), einheitliche **`[SOS]`**-Anzeige (Mailbox + Mesh). |
