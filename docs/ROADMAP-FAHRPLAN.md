...
**Aktuell sinnvolle „nächste drei“ konkrete Arbeiten (Stand Doku 2026-03-31):** (1) **§ H.2** — Vorprüfung A–C + Protokoll/Tabelle **Lücken** in **`docs/PWA-MANUAL-CHECKS.md`**; **Punkt 6:** kanonische Fetch-/Inbox-Offline-Strings (**`api-fetch-text`** ↔ **`inbox-load-error`**, 2026-03); Feld-Checks **#1–5** weiter manuell. (2) **§ H.1b** — API-Einstieg nur noch **`@/frontend/lib/api`**; **Re-Export `frontend/lib/api.ts` entfernt** (2026-03-30). (3) **§ H.3g** Pakete **1–2–6**: Next-PWA **Einsatz-Rollen-Vorlagen** (Einstellungen) + **Handbuch-/SW-Offline** für **`API-EINSATZ-ROLE-TEMPLATES.md`**; **Paket 7 voll** (Offline-Relay / Settlement) erst mit **§ H.12** / Phase B — **Paket 7a** (Client-Mailbox-Outbox, Opt-in `morgendrot.offlineMailboxQueue`) ist **Vorbereitung** und **kein** Ersatz dafür. **Kürzlich umgesetzt / eingeordnet:** **§ H.6c** zusätzliche Bestätigung vor **Einsatz-Exporten**, wenn **`deviceTimeTrustWarn`**; **Sparse-Luma**-Sende-Fortschritt (**Mesh v2** LUMA/CHROMA-Paketzähler in der Anhang-Leiste); **§ H.3g 7a** Client-Mailbox-Outbox (**`lib/api/offline-queue.ts`**, Send-Hook + Status-Poll-Drain, Opt-in; **`timeIsTrusted`** / § **H.6c**, **`clientOutSeq`**); **§ H.6c** Empfangs-**Unvollständig**-Badge + Kompakt-Luma-Fallback; **§ H.2** Punkt 6 kanonische Fetch-/Inbox-Offline-Texte (**`api-fetch-text`** ↔ **`inbox-load-error`**); **§ H.8** Dokuabgleich **§ C.0b** Stufe 2 (**`DIENST-VS-PRIVAT-NETZ-PROFIL.md`**, Tabelle unten, **`PROJECT-FOCUS-AND-PRIORITIES.md`** Stand); **§ H.10** Sicherheits-/Vertrauens-Track (**`ROADMAP-SICHERHEIT-VERTRAUEN-UND-SCHLANKHEIT.md`**, Verweis **C.0b** + Schritte **A**); **§ H.10b** (**`BOSS-WORKER-SEED-CUSTODY.md`**, **C.0b**-Einleitung); **§ H.3g** Paket **6** Client-Validierung Einsatz-Rollen-Vorlagen (**`einsatz-role-templates-validate.ts`**, Settings vor `POST`). **Phase B:** Spec **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** (**§ H.3n**) vor größeren Eingriffen mit **`emergency-binary-wire.ts`** und **`LORA-IOTA-DELAYED-UPLOAD-SPEC`** abgleichen.
