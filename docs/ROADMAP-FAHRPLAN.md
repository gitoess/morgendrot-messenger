...
**Als Nächstes (klein, empfohlen):** (a) Weitere ESLint-Zonen nur ergänzen, wenn **messbare** Querimports entstehen (z. B. **attachments**↔**inbox**); (b) **`frontend/lib/api.ts`-Re-Export** entfernen, sobald keine externen Altimporte mehr nötig sind; (c) § **H.1a** weiterer RTL-/Vitest-Slice am **Send-Panel** / Inbox-Rand, wenn Mesh-Kern nicht parallel stark bewegt wird.
