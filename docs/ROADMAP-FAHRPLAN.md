...
**Nächste konkrete Schritte (2026-06-02):** **§ H.33** Code **Ist** (Move + UI + API). **Jetzt:** **Feld-/Smoke** — **`docs/HANDY-FIRST-STAGE2-CLIENT-SUBMIT-SMOKE.md`** § 2/§ 4b–4f, **`docs/EINSATZ-MANIFEST-API-SMOKE.md`**, Consumer-Retest (#1/#3/#4 mit 2 Wallets), **`docs/HANDY-LATER-MANUAL-TESTS.md`**. **Parallel (klein):** **§ H.1a** Vitest-Slices. **Backlog:** **§ H.24** Package-Profile; **§ H.28** Discord/Matrix; LoRa **Ticket A–C** (**§ H.3** Phase B); **§ H.26** B5 `/nodes`/`/qr`.
...
| **§ H.36** | **Team-Member-Update & Einstiegs-Wizard** — linearer Erststart, Join-Request, Transport **IOTA = Persistenz / LAN = Zustellung** — **`docs/TEAM-MEMBER-UPDATE-WIZARD-SPEC.md`** (nutzt § H.16 Boss-LAN Ist) |
...
| **Ist-Code (Messenger)** | **B1+B2:** UI-SOS, `MORG_EMERGENCY_V1` (**§7** Freeze), Mesh-Flash-Burst, **Backoff-Retry** + **Mailbox-Ack stoppt Funk** (`morg-sos-mesh-retry`, `/send` zwischen Versuchen; Opt-out `localStorage`), **IOTA-Spiegel** nach reinem Funk-Erfolg, einheitliche **`[SOS]`**-Anzeige (Vitest: `chat-message-display-normalize`). **Lücke §9:** SOS erzwingt noch nicht **`encrypted=false`** / **`/send-plain`**; kein Fan-out-Dialog. |
| **Nächste Scheibe (B2.5)** | **`ChatViewSosEmergencySheet`**: Lage-Bundle (Text, Ort, Name, IDs), Fan-out, Transparenz-Hinweis; Send-Pfad SOS → Klartext erzwingen. |
...
| **Priorität** | **B2.5** (SOS-Sheet + Klartext erzwingen) **nach** stabilen Chat-UI-Scheiben, **vor** großem Wire-v2; **Phase B** Basis-Queue (**B3**) parallel planbar — **nicht** vor **§ C.0b** Stufe 4-Voraussetzungen; **nicht** parallel zu großem **§ H.1b**-Refactor ohne Absprache. |
...
| **SOS Zielbild §9** (Zivil Klartext-only) | **Entschieden 2026-06-16:** SOS unverschlüsselt, Fan-out, Lage-Bundle — **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** §9; UI **B2.5**; OPSEC-verschlüsselt = Backlog. |
...
**Status:** **Teil umgesetzt** (2026-05-17, **B2 Long Polling 2026-05-20**, **B3 Opt-in 2026-05-28**, **B5 `/help`/`/status` 2026-06-02**). **Ist-Code:** Monitor-Alarme → **`scripts/telegram-webhook.ts`** / Relay; Runtime **`src/integrations/telegram-integration.ts`**, API **`/api/integrations/telegram`**, UI **Einstellungen → Telegram**, Journal **`.morgendrot-telegram-journal.json`**, Posteingang-Merge; **Eingang:** **`src/integrations/telegram-inbound-poll.ts`** (Long Polling, **Phase B2**); **Sende-Hinweis:** B3 Opt-in; **Bot-Kommandos:** B5 **`telegram-bot-commands.ts`**. Spez **`docs/TELEGRAM-INTEGRATION-ZIELBILD.md`**. **Offen:** B4b Boss-Gruppenalarm; B5 `/nodes`/`/qr`; kein Chat-Vollspiegel.
...
### H.36 — Team-Member-Update & Einstiegs-Wizard (**Spec 2026-06-16**)

**Status:** **Spec** — **`docs/TEAM-MEMBER-UPDATE-WIZARD-SPEC.md`**. **P0** Wizard-Skelett als Nächstes; **kein** Duplikat von Helfer einrichten / Handoff.

| Phase | Inhalt | Abhängigkeit |
|-------|--------|--------------|
| **P0** | Linearer Wizard Boss / Helfer / Wanderer; Skip-Logik; Deep-Links (Handoff, Seed-QR, WLAN-QR § H.16, Funk) | Bestehende Panels |
| **P1** | `MORG_TEAM_MEMBER_UPDATE_V1`; Posteingang „Neues Mitglied“ + Ja/Nein; **LAN-Push** wenn Basis-URL = Boss-LAN; **IOTA** parallel (Persistenz); UI Kanal-Feedback | § H.16 Relay Ist |
| **P2** | `MORG_TEAM_JOIN_REQUEST_V1`; Boss Pending-Liste; Freigabe → Update; optional mDNS | P1 |
| **P3** | Funk-Ping `MORG_TEAM_UPDATE_PING_V1`; Boss-`sig` freeze | P1 |
