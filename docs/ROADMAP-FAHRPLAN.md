...
**Nachtrag 2026-06-16 (§ H.36 — Team-Member-Update & Einstiegs-Wizard):** Spec **`docs/TEAM-MEMBER-UPDATE-WIZARD-SPEC.md`**: geführter linearer Wizard (Boss/Helfer/Wanderer) orchestriert Handoff/Seed/Funk **ohne** zweiten Export-Assistenten; spontaner **Join-Request**; Wire `MORG_TEAM_MEMBER_UPDATE_V1` + Empfänger **Ja/Nein**; Transport **IOTA = Persistenz**, **LAN = Zustellung** (Boss-Relay Ist via § H.16), Funk nur Ping. **P0** Wizard-Skelett → **P1** Wire+LAN+IOTA → **P2** Join-Queue.
...
**Nachtrag 2026-06-02 (§ H.33 API):** Boss/Kommandant/Werkstatt: **`GET /api/einsatz-manifest/config|anchors|probe`** — Mainnet-Anker ohne Browser-Direct-RPC; UI-Fallback in `einsatz-manifest-anchors-list.ts` / `einsatz-manifest-on-chain-probe.ts`. Doku **`docs/API-EINSATZ-MANIFEST.md`**, Smoke **`docs/EINSATZ-MANIFEST-API-SMOKE.md`**.

**Nachtrag 2026-06-02 (Einsatzleitung — Helfer einrichten, Ist):** Kanon: **`docs/EINSATZ-HELFER-EINRICHTEN-ZIELBILD.md`**, Boss-Ablauf **`docs/EINSATZ-BOSS-ABLAUF.md`**. **UI:** Einsatzleitung = Hub-Titel → **Helfer einrichten** (eine Karte: Handoff compact + Capabilities-Matrix + Team/Partner + **ZIP / IOTA / WLAN-QR** + Seed-Block + **Bestehende Geräte** TTL/Purge) → **Erweitert** (nur Chain/Move). **Kein** Schritt 1/2, **keine** grüne Summary-Box, **keine** langen Hilfetexte; **TTL nur** im Block „Bestehende Geräte“ (nicht im Export-Experten). **WLAN-QR:** neben IOTA (nicht unter Erweitert); **`GET /api/lan-install-urls`** ermittelt LAN-IP automatisch (**ohne** Terminal/`npm run dev:lan` im Dialog). **Presets:** Medic-Funker / Reporter in Matrix-Schnellprofilen + Wizard. **Phase 2–4 Ist:** ZIP + Seed + QR; Capabilities UI + Backend-Gate; **Vorlagen** mit vollem `handoffSnapshot` (Capabilities, Partner, Team). **§ H.33** Move/UI/API **Ist** (Deploy Mainnet = Admin). **Doku-Sync:** **`docs/EXPORT-ASSISTENT-REFERENZ.md`**, **`docs/MESSENGER-CHAT-HANDBUCH.md`** § Einsatzleitung.

**Nachtrag 2026-07-07 (Block 2 Feldtest, APK-Mobile-Shell, Boss-Inbox):**
...
| Bereich | Ist (Code) | Offen |
|---------|------------|-------|
| **Chat-Navigation** | **Sidebar-first:** Kanal/Empfänger in **`chat-view-contact-sidebar`** (1:1, Gruppe, Pinnwand, Telegram); Sendepfad **`chat-view-send-path-compact`** nur noch **Transport** (Online/Funk/Telegram) — **entfernt:** `chat-view-send-path-recipient-picker`, `messenger-send-path-recipient-options`; Header-Kontext **`resolve-chat-header-context.ts`** | Feldtest Sidebar-Flows (Privat/Gruppe/Pinnwand/Telegram) |
...
| 6 | **Gruppenchat „Mailbox an alle Mitglieder“** | M2c Team-Broadcast (1× TX); pairwise-Checkbox entfernt | **Ist 2026-06-02** (Gruppe + Internet + Mailbox = Team-Postfach; `group-team-broadcast.ts`; Consumer wie Einsatz-Rollen) |
| 7 | **§ H.25a** Flüchtig-LoRa-Bild Feldtest | Zwei Heltecs, Vitest grün | **Vitest Schreibtisch ✅** — Feld offen |
| 8 | **§ H.15 Stufe 2** Handy-first Smoke | `docs/HANDY-FIRST-STAGE2-CLIENT-SUBMIT-SMOKE.md` **4b–4f** (Code Standalone/APK) | **Teil-PASS (Feld 2026-07-07)** — **4b–4d** + Helfer→Boss grün (1 Gerät); **4e/4f** offen |
...
| 15 | **Boss bei 0 — Modus A** (Wizard 8 Schritte + Readiness) | **`docs/FELDTEST-BOSS-BEI-0.md`**; Gate vor **§ H.23** | **PASS (Feld 2026-07-03)** — Logbuch **`docs/TEST-RUN-LOGBOOK.md`** |
...
| **G — Wanderer / Consumer** | Privat, kein Boss | Messenger schlank | Konfiguration selbst | Presets + Handshake/Team-Beitritt **Ist** | `DEPLOYMENT_PROFILE=consumer` | Feld-Retest (#1/#3/#4) |
...
| **Offen** | Mainnet-Registry **Deploy** (Admin, `DEPLOY-MOVE-H33`); Feld-Smoke Anker/API |
...
| **Offen** | Feld-Retest Handshake empfangen (#1), Team-Beitritt (#3), Sendepfad (#4) — Smoke-Dokus bereit |
...
| **§ H.16** | **Telefonbuch, QR, Boss-LAN** — Kontakt-Dialog schlank (**2026-06-17**), Funk-Export-Vorschau; **`docs/QR-CONTACT-SCHEMA-V2.md`** (**§ H.3b**); **Einsatzleitung compact:** **`docs/EINSATZ-HELFER-EINRICHTEN-ZIELBILD.md`** |
...
| **§ H.36** | **Team-Member-Update & Einstiegs-Wizard** — **P0 Ist**, **P1/P2 Teil-Ist** — **`docs/TEAM-MEMBER-UPDATE-WIZARD-SPEC.md`** (nutzt § H.16 Boss-LAN Ist) |
...
- **`docs/NOTFALL-REICHWEITE-BRUECKEN-UND-BACKLOG.md`** – Notfall: Gegenstellen, 112-Brücke; **§3.1** Zivil-SOS Klartext-only (dedizierter Button); kein generischer LongFast-Dauerchat.
...
**Status:** **weitgehend erledigt** (Doku final, Config/Status, UI-Gates, Handoff+PSK, Chat-Feinschliff, Vitest-Slices, **Sidebar-Navigation 2026-06-17**, **APK Mobile-Shell 2026-07-07**). **Offen:** Block 2 §3 Simple-Checks; **4e/4f** (2. Gerät); **§ H.36 P1** LAN-Push. **Phase B** (Delayed Upload) bewusst **Backlog**.  
...
| **3 (UI)** | Package-Banner, Relay, IOTA-Filter, Sendepfad-Matrix aus Simple Mode; **Chat: Sidebar = Empfänger** (2026-06-17) | **Ist 2026-05-20** + Navigation-Nachtrag **2026-06-17** |
...
**Nachgezogen (2026-06-17):** **Sidebar-first Chat-Navigation** — Empfänger/Kanal in **`chat-view-contact-sidebar`**; Sendepfad nur Transport; **`resolve-chat-header-context.ts`**; Pinnwand als Sidebar-Zeile. **Scope-B lazy:** **`lazy/messenger-scope-b.ts`** (Wellen 1–4, Checkliste **`docs/MESSENGER-SCOPE-P0-LAZY-PLAN.md`**). **Composer-Ports:** **`send-actions-port`**, **`composer-partner-port`** (Refactor-Slice). Vitest: **`contact-mesh-export-stats.test.ts`**, **`resolve-chat-header-context.test.ts`**, **`chat-view-send-path-compact.test.tsx`**.
...
| **Produktentscheidung (2026-06-16)** | **Zivil-SOS = Klartext-only**, Reichweite zuerst — **kein** Verschlüsselungs-Schalter im SOS-UI. Fan-out über **alle erreichbaren** Wege (Funk, Online **`/send-plain`**, Telegram Notify). Details **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** **§9**. Verschlüsseltes SOS nur **Backlog** (OPSEC-/Militär-Profil). |
| **Doku** | **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** — Idealbild, **§9 Zivil-SOS**, Korrekturen (Meshtastic priorisiert nicht von selbst; **keine** volle IOTA-Signatur über Funk; **Duty Cycle**; Sprache = Hash/Chunks), Abgrenzung normaler Chat vs. Emergency, Phasen B1–B4 + **B2.5** (SOS-Sheet). |
| **Ist-Code (Messenger)** | **B1+B2:** UI-SOS, `MORG_EMERGENCY_V1` (**§7** Freeze), Mesh-Flash-Burst, **Backoff-Retry** + **Mailbox-Ack stoppt Funk** (`morg-sos-mesh-retry`, `/send` zwischen Versuchen; Opt-out `localStorage`), **IOTA-Spiegel** nach reinem Funk-Erfolg, einheitliche **`[SOS]`**-Anzeige (Vitest: `chat-message-display-normalize`). **B2.5 Teil-Ist (2026-06-17):** **`ChatViewSosEmergencySheet`** + Lage-Bundle (`sos-lage-bundle.ts`); SOS auf **Dashboard** + Chat (`emergencyWire`); Telegram-Gruppen-Hinweis bei SOS (`postTelegramGroupAlarm`). **Lücke §9:** normales Composer-SOS erzwingt noch nicht pauschal **`encrypted=false`** / **`/send-plain`**. |
| **Nächste Scheibe (B2.5 Rest)** | Sendepfad SOS → **Klartext erzwingen** (auch bei aktivem Schloss); Fan-out-Feldabnahme (Funk + Online + Telegram). |
...
| **Priorität** | **B2.5 Rest** (Klartext erzwingen + Fan-out-Feldtest) **nach** Sidebar-/Telefonbuch-Abnahme; **Phase B** Basis-Queue (**B3**) parallel planbar — **nicht** vor **§ C.0b** Stufe 4-Voraussetzungen; **nicht** parallel zu großem **§ H.1b**-Refactor ohne Absprache. |
...
| **H.3o.4** | **UI Sendepfad-Matrix** | **Gruppe + Funk ✓**, **Pinnwand + Funk ✗** (IOTA-Brett ≠ Primary-Broadcast). Empfänger/Kanal über **Sidebar**, Sendepfad nur Transport (**2026-06-17**). Siehe **`docs/MESSENGER-CHAT-HANDBUCH.md`** § Sendepfad. |
...
| **SOS Zielbild §9** (Zivil Klartext-only) | **Entschieden 2026-06-16:** SOS unverschlüsselt, Fan-out, Lage-Bundle — **`docs/MORG-EMERGENCY-SOS-WIRE-SPEC.md`** §9; **B2.5 Teil-Ist** (SOS-Sheet); Klartext-Erzwingung im Sendepfad **offen**; OPSEC-verschlüsselt = Backlog. |
...
| **Boss-PC im lokalen WLAN** | **Szenario:** Boss läuft im LAN (Standalone/PWA auf dem Einsatz-PC); Helfer scannen **WLAN-QR** in **Helfer einrichten** → PWA installieren. **Ist:** `GET /api/lan-install-urls` + Payload `k: "mi"` (**`frontend/lib/install-qr.ts`**). **Kein** Terminal-Schritt im Dialog. Handoff **separat** (ZIP/IOTA). **Kritisch:** Firewall (typ. 3341/3342), ggf. `API_BIND_HOST=0.0.0.0`; Cleartext HTTP für APK. **Doku:** **`docs/EINSATZ-HELFER-EINRICHTEN-ZIELBILD.md`**, **`docs/PWA-HANDBUCH-OFFLINE.md`**. **Soll (§ H.36 P1):** dedizierter **LAN-Push** für Team-Update (heute: IOTA über **`sendPlaintextMailboxHybrid`** + Boss-Relay indirekt) — **`docs/TEAM-MEMBER-UPDATE-WIZARD-SPEC.md`** §8. |
| **Telefonbuch UX (2026-06-17)** | **Ist:** Kontakt-Dialog schlank; Postfächer unter **Erweitert** (ein Privat-Feld + optional Team/Einsatz/Puffer); **Funk · Node-IDs sichern** am Sheet-Ende mit Vorschau/Toast (`contact-mesh-export-stats.ts`); Erklärtexte ins **Handbuch** verschoben. **Entfernt:** „Wallet aus Chain“ (Frontend-Helfer `fetchResolvePrivateMailboxOwner`); API **`/api/resolve-private-mailbox-owner`** bleibt optional. | Feldtest anlegen/bearbeiten/QR; Funk-Export mit/ohne Node-IDs |
...
**Nachtrag 2026-06-17 (M4e UI):** Kontakt-Dialog — vier Slots nur noch unter **Erweitert** (Privat sichtbar, Team/Einsatz/Puffer per Link); Composer-**Ziel-Postfach** unverändert. Doku **`docs/MESSENGER-CHAT-HANDBUCH.md`** § Kontakt anlegen.
...
**Status:** **Teil umgesetzt** (2026-05-17, **B2 Long Polling 2026-05-20**, **B3 Opt-in 2026-05-28**, **B5 `/help`/`/status` 2026-06-02**, **B4b Teil-Ist 2026-06-17**). **Ist-Code:** Monitor-Alarme → **`scripts/telegram-webhook.ts`** / Relay; Runtime **`src/integrations/telegram-integration.ts`**, API **`/api/integrations/telegram`**, UI **Einstellungen → Telegram**, Journal **`.morgendrot-telegram-journal.json`**, Posteingang-Merge; **Eingang:** **`src/integrations/telegram-inbound-poll.ts`** (Long Polling, **Phase B2**); **Sende-Hinweis:** B3 Opt-in; **Bot-Kommandos:** B5 **`telegram-bot-commands.ts`**. **B4b Teil-Ist:** Einsatz-Alarmgruppe in Einstellungen, Handoff-Extras, **`MORG_TELEGRAM_ALARM_GROUP_V1`**, Join-Karten, **`postTelegramGroupAlarm`**, Inbound-Gruppe ohne Telefonbuch. Spez **`docs/TELEGRAM-INTEGRATION-ZIELBILD.md`**. **Offen:** B4b Feld-Onboarding (Einladungslink-Flow); **B4c** Telefonbuch Multi-Pick; B5 `/nodes`/`/qr`; kein Chat-Vollspiegel.
...
| **B5** | Bot-Kommandos `/help`, `/status` | **Ist** (`telegram-bot-commands.ts`) — `/nodes`, `/qr` Backlog |
...
##### Phase B5 — Bot-Kommandos (Ist)

- **`/help`**, **`/status`** — Antwort per `sendMessage`; Telefonbuch-Chat-ID oder **`adminChatId`**.
- Kein Journal-Eintrag für Kommandos (Ops-Hilfe, kein Chat-Spiegel).

**Abnahme B5:** Long Poll aktiv → `/status` liefert Rolle + Modus; fremde Chat-ID → keine Antwort.
...
| **B4b** | **Boss-Gruppenalarm (Telegram)** | **Teil-Ist 2026-06-17** | **Ein** Boss-Bot + **eine** Einsatz-Gruppe; Wire + Einstellungen + Join-Karten + SOS/Team-Hinweis — **`docs/TELEGRAM-INTEGRATION-ZIELBILD.md`** **§6**; Feld-Onboarding Einladungslink offen |
...
| Bot-Kommandos (`/help`, `/status` **Ist**; `/nodes`, `/qr` Backlog) | tb0hdan | Ops-Hilfen ohne UI-Zwang | **mittel** |
...
|---|---|---|
| **B5.1 Core-Bridge** | Node-Routing, Mapping, Minimal-Kommandos | **`/help` + `/status` Ist** (`telegram-bot-commands.ts`); `/nodes`/`/qr` offen |
...
2. **Dazwischen:** Standalone-Smoke, Consumer-Feldtests — **§ H.33** Move/UI/API **Ist** (2026-06-02).
...
**Status:** **Ist 2026-06-02** — `EINSATZ_CHAIN_MODE` im Handoff + Export-Banner, Manifest-Builder, Verifikation, PTB + Boss-UI (**Mainnet-Registry anlegen**, **Auf Mainnet ankern**, **Mainnet-Anker auflisten/prüfen**), API **`GET /api/einsatz-manifest/*`**. Move in `messaging.move`; **Deploy Mainnet** = Admin (`DEPLOY-MOVE-H33`) oder UI-Registry-Button.
...
| **4 — UI Boss** | Export-Assistent: Modus **A/B/C**; Dialog Anker, Kosten, Explorer | **Ist** (Modus-Banner Export, Mainnet-Registry/Anker UI, Explorer, Kosten, Einsatz-Ende-Anker, **Mainnet-Anker auflisten**, API **`GET /api/einsatz-manifest/*`** — **`docs/API-EINSATZ-MANIFEST.md`**) |
...
**Abhängigkeiten:** **`@morgendrot/core`** PTB-Builder; **`GET /api/einsatz-manifest/*`** (**Ist** — `docs/API-EINSATZ-MANIFEST.md`); **§ H.32b** liefert sauberen Schnitt „Einsatz zu“.
...
| 6 | **§ H.32b** + Anker (A): Einsatz beenden **löscht nicht** Mainnet-Anker; lokaler Index reset | **Ist** (`einsatz-end-cache-wipe.ts` — `manifestLastSequence`, Anchor-Cache-Keys) |
...
### H.36 — Team-Member-Update & Einstiegs-Wizard (**Spec 2026-06-16**, **P0/P1 Teil-Ist 2026-06-17**)

**Status:** **`docs/TEAM-MEMBER-UPDATE-WIZARD-SPEC.md`**. **P0 Wizard-Skelett Ist**; **P1 Wire + Posteingang + Join-Queue Teil-Ist** — **kein** Duplikat von Helfer einrichten / Handoff.

| Phase | Inhalt | Status |
|-------|--------|--------|
| **P0** | Linearer Wizard Boss / Helfer / Wanderer; Skip-Logik; Deep-Links (Handoff, Seed-QR, WLAN-QR § H.16, Funk, Telegram Schritt 2) | **Ist 2026-06-17** — `onboarding-wizard-*`, `onboarding-progress-store.ts`, `OnboardingWizardHost` in **`messenger-dashboard.tsx`**, **`settings-onboarding-section.tsx`** |
| **P1** | `MORG_TEAM_MEMBER_UPDATE_V1` + **`MORG_TELEGRAM_ALARM_GROUP_V1`**; Posteingang „Neues Mitglied“ + Ja/Nein (**`InboxTeamSyncSystemCards`**); IOTA-Klartext Team-Mailbox (**`team-sync-wire.ts`**); Funk-Ping **`MORG_TEAM_UPDATE_PING_V1`**; Offline-Queue Boss | **Teil-Ist** — **offen:** dedizierter **LAN-Push**, UI „Zugestellt über LAN/IOTA/Funk“, Boss-**`sig`** freeze |
| **P2** | `MORG_TEAM_JOIN_REQUEST_V1`; Boss Pending-Liste; Freigabe → Update; optional mDNS | **Teil-Ist** — **`helper-join-request-form`**, **`einsatzleitung-join-requests-panel`**; mDNS offen |
| **P3** | Funk-Ping produktiv + Boss-`sig` freeze | **Teil-Ist** (Ping-Wire + Side-Effects in **`team-sync-wire.ts`**) |

**Transport (verbindlich):** IOTA = authoritative Speicher; LAN = bevorzugte **Zustellung** wenn Boss erreichbar; **Ad-hoc ≠ WLAN** (`docs/TRANSPORT-AND-IOTA-LAYERS.md`).
