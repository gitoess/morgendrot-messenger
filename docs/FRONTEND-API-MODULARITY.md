...
- **`features/send/`** — Mesh-Burst, Send-Validierung, Outgoing-Wire, `.txt`-Split; **`lib/chat-view-mesh-send.ts`**, **`lib/chat-view-send-utils.ts`**, **`lib/chat-view-outgoing-payload.ts`**, **`lib/chat-view-txt-split.ts`** re-exportieren für alte Importe.
