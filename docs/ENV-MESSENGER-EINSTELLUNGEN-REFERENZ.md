# Messenger — `.env`-Einstellungen (Hauptreferenz)

**Stand:** 2026-06-02  
**Status:** **Kanonische Hauptreferenz** für alle Konfigurationsschlüssel, die in **Morgendrot Messenger → Einstellungen** sichtbar sind (System & Identität, inkl. **Erweiterte Konfiguration (.env)**).

**Umfang:** **77 Keys** — `RPC_URL` und `PACKAGE_ID` (eigene Zeilen über der Liste) plus **75** Einträge in `CONFIG_KEYS_MESSENGER` (`frontend/frontend/lib/config-env-field-meta.ts`). Pro Key Kurztext in der App aus `META`; Details hier und in den Kategorien unten.

**Quelle im Code (bei Abweichung gilt der Code):** `CONFIG_KEYS_MESSENGER`, `CONFIG_KEYS_EDITED_ABOVE`, `META`, `shouldShowConfigKeyInMessenger`.
...
## Alphabetisches Verzeichnis (alle 77 Keys)

`AUTHORIZED_SENDERS` · `BOSS_ADDRESS` · `BOSS_SIGNER_PUBLIC_URL` · `BROADCAST_AUTHORIZED_SENDERS` · `BROADCAST_PINNWAND_ADDRESS` · `DEFAULT_KEY_TTL_DAYS` · `DEFAULT_TTL_DAYS` · `DEPLOYMENT_PROFILE` · `DEVICE_NAMES` · `DEVICE_ROLES` · `ENABLE_AUTO_EXECUTE` · `ENABLE_BROADCAST_PINNWAND` · `ENABLE_COMMAND_DOWN` · `ENABLE_CONFIG_CHANGE` · `ENABLE_FETCH_COMMAND` · `ENABLE_HD_CONTACT_ADDRESSES` · `ENABLE_HEARTBEAT` · `ENABLE_HIERARCHY_CHANGE` · `ENABLE_KEY_ISSUE` · `ENABLE_LISTENER` · `ENABLE_PAIRWISE_GROUPS` · `ENABLE_PLAINTEXT_CHANNEL` · `ENABLE_PURGE` · `ENABLE_REPLAY_PROTECTION` · `ENABLE_REVOKE_DOWN` · `ENABLE_STATUS_READ_DOWN` · `ENABLE_STATUS_READ_UP` · `FETCH_LAST_ON_START` · `GAS_BUDGET` · `GAS_STATION_ENABLED` · `GAS_STATION_MIN_IOTA` · `GAS_STATION_TOPUP_IOTA` · `HANDSHAKE_REFRESH_MS` · `HEARTBEAT_INTERVAL_MS` · `IOTA_GAS_STATION_URL` · `KOMMANDANT_ADDRESSES` · `LISTENER_POLL_MS` · `MAILBOX_ID` · `MAILBOX_STORE_PLAINTEXT` · `MAX_SEND_AMOUNT_IOTA` · `MESSENGER_AUTO_SPONSOR` · `MESSENGER_CREDITS_OBJECT_ID` · `MESSENGER_EDITION` · `MESSENGER_GAS_STATE_FILE` · `MESSENGER_LICENSE_NFT_OBJECT_ID` · `MY_ADDRESS` · `NETWORK_TRUST_TIER` · `OPEN_STREAMS_ENABLED` · `PACKAGE_ID` · `PAIRING_GATE_NFT_OBJECT_ID` · `REMOTE_SIGNER_TOKEN` · `REMOTE_SIGNER_URL` · `REPLAY_STATE_FILE` · `ROLE` · `ROLE_ID` · `RPC_HTTP_PROXY` · `RPC_SOCKS_PROXY` · `RPC_URL` · `RPC_URLS` · `SHADOW_SWEEP_GAS_RESERVE_MIST` · `SIGNER` · `SIMPLE_MODE` · `SPONSOR_GAS_OWNER` · `SPONSORED_TRANSACTION_ENABLED` · `STREAMS_ANCHOR_ID` · `STREAMS_BRIDGE_URL` · `STREAMS_LISTEN_ENABLED` · `STREAMS_TOPIC` · `TRANSPORT_PROFILE` · `UI_VARIANT` · `USE_ENCRYPTED_DISCOVERY` · `USE_MAILBOX` · `VAULT_FILE` · `VAULT_REGISTRY_ID` · `VERIFIED_IOTA_NAME_PACKAGE_IDS` · `WALLET_DERIVATION_PATH` · `WORKER_ADDRESSES`

*(Sortiert alphabetisch; ausführliche Tabellen in den Abschnitten darunter.)*
...
| **HEARTBEAT_INTERVAL_MS** | nein | Abstand zwischen Heartbeats (ms) |
...
**Nicht in der Einstellungs-Key-Liste (nur Handoff-ZIP / Build):** `NEXT_PUBLIC_DIRECT_IOTA_RPC_URL` (PWA Direkt-IOTA, oft per Handoff oder Build), `TEAM_MAILBOX_IDS`, `HANDOFF_LABEL` — siehe **`docs/EXPORT-ASSISTENT-REFERENZ.md`**.
...
*Pflege: Neue Keys zuerst in `config-env-field-meta.ts` (`CONFIG_KEYS_MESSENGER` + `META`), dann diese Datei und `npm run sync:handbook` (Kopie unter `frontend/public/handbook/`). Vitest: `frontend/lib/config-env-field-meta.test.ts`.*
