...
4. **Empfang ohne `/api/fetch`** — Inbox aus der Kette/RPC lesen (owned objects / dokumentierte Read-Pfade), Pagination + Dedup mit **§ H.12** / **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** § 8. **Ist (2026-06):** `fetchMailboxInboxRpcRows` + **`fetchMessagingEventInboxRpcRows`** (Move-Events `EncryptedMessage`/`PlaintextMessage`); **`tryFetchDirectMailboxInboxViaIota`** merged Mailbox + Events (Shared-Union), Entschlüsselung im Browser. **`use-chat-view-inbox`:** **RPC vor API**. Private Mailbox-Only-Fetch ohne Event-Union (wie API).
5. **Peering (Handshake/Connect)** — **Ist (2026-06):** Hybrid **Direkt-RPC vor API** (Send, Connect, **Purge** `purge-handshake-hybrid`); **Peering-QR**; Standalone ohne Relay (`messenger-standalone-relay.ts`). **Relay-Modus** bleibt opt-in.
6. **Abgabe & Erwartungshaltung** — **Ist (2026-06):** **`docs/WANDERER-STANDALONE-BUNDLE.md`** Variante **B** (APK, Handoff lokal, Peering-QR, Smoke **4b–4f**); Dashboard-Hinweis ohne „`npm run dev`“-Pflicht im Standalone. **Offen:** **`docs/HANDY-FIRST-STAGE2-CLIENT-SUBMIT-SMOKE.md`** manuell abnehmen.
