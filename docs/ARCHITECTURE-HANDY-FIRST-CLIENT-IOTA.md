...
4. **Empfang ohne `/api/fetch`** — Inbox aus der Kette/RPC lesen (owned objects / dokumentierte Read-Pfade), Pagination + Dedup mit **§ H.12** / **`docs/SYNC-SOURCE-OF-TRUTH-UND-KONFLIKTE.md`** § 8. **Ist (Stufe 1):** Klartext-Mailbox-Einträge per Fullnode (`fetchPlaintextMailboxInboxRows` in **`@morgendrot/core`**, Hook **`use-chat-view-inbox`** nutzt den Pfad, wenn Snapshot + Flags wie Klartext-Direkt-Senden und mindestens eine Zeile zurückkommt — sonst `/inbox`). Verschlüsselte Mailbox + reine Event-Pfade folgen.
