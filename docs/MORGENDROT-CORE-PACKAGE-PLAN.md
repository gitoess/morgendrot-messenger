...
3. **Erledigt (2026-03-28):** § **H.12** / **`canonical_msg_ref`** (v1 gemäß **`LORA-IOTA-DELAYED-UPLOAD-SPEC.md`**) im Core + PWA-Enqueue (`senderAddress`, `threadId`, **`messageNonceU64`** = Compose-Snapshot **`nextOfflineMailboxClientOutSeq()`**). Drain: **`OfflineMailboxTrySend`**, **`runOfflineMailboxDrainCycle`**, **`createOfflineMailboxTrySendFromSendPort`**; Manager-**`drainOnce`** akzeptiert Port oder Callback.  
