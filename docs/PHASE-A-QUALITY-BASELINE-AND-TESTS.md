...
| **Git + Realworld + Doku** | **Regelmäßig** (z. B. wöchentlich oder vor Tag): definierte **`npm run test:messages`** / **`test:realworld`**-Teilmenge laut **`TESTING.md`**, Ergebnis kurz festhalten (Commit-Message, **`docs/CHAT-PROTOKOLL-*`** oder Operations-Notiz); **kleine Commits** halten den Code **stabil** und **bisect-fähig**. |
