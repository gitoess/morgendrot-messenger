...
| 2026-04-22 | ✓ | ✓ (15 Dateien) | ✓ | **§ C.0b / § H.2 Schreibtisch-Ritual:** **`npm run check:pwa-desk:full`** komplett grün (`build:pwa-icons`, `sync:handbook`, `frontend build`/Next 16.1.6, 7 statische Routen, `[check-pwa-manual-desk] OK`). Git-Stand seit 2026-04-21 auf `main` gepusht; SW/Handbuch-Parität mit **`morgendrot-sw-14`** aktiv. Geräte-Checks **L1–L5** weiter separat. |
| 2026-04-21 | — | ✓ (15 Dateien) | empfohlen | **§ H.2 / D:** **`VAULT-EINRICHTEN.md`**, **`VAULT-BEGRIFFE-MESSAGEN-vs-TRESOR.md`** in **`scripts/sync-pwa-handbook.mjs`** + **`handbook-client.tsx`**; **`sw.js`** → **`morgendrot-sw-14`** + **`HANDBOOK_URLS`** (Parität mit Sync-Liste). UI: Tresor/Einstellungen verlinken Handbuch statt Langtext. **Git:** Änderungen sind inzwischen auf `main` committed und gepusht. |
