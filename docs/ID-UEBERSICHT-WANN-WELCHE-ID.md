...
| ID | Bedeutung | Wo in der UI | Historie? |
|----|-----------|--------------|-----------|
| **UpgradeCap-ID** | Der „Generalschlüssel“ für das Package: Nur wer diese Object-ID besitzt, darf das Package upgraden (Code reparieren/ändern). Wird beim ersten Deploy erzeugt; gehört dem Wallet, das deployt hat. | **Aktuell nicht in der UI.** Nach Deploy vom Explorer oder aus Chain-Abfrage ermittelbar. | Nein. |
...
- **UpgradeCap-ID:** Nicht in der UI. Sinnvoll: nach Deploy anzeigen (wenn Backend/Chain es liefert) und Hinweis „Im Tresor sichern“. Bis dahin: manuell aus Explorer/Events notieren.
