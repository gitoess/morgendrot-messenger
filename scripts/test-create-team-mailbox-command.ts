/**
 * Smoke: /create-team-mailbox gegen laufende API (Tresor entsperrt, neues PACKAGE_ID).
 * Usage: UNLOCK_PASSWORD=… tsx scripts/test-create-team-mailbox-command.ts
 */
...
async function main() {
    ...
    const res = await fetch(`${API}/api/command`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cmd: '/create-team-mailbox', args: [] }),
    });
    ...
    if (body.objectId) {
        console.log('\nTeam-Mailbox Object-ID (für Gruppe + Handoff):', body.objectId);
    }
}
