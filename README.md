...
1. **`docs/DEV-START.md`** — Lokales Setup, Rollen, die zwei Web-Oberflächen (**`frontend/`** = Next-Kundenprodukt, **`ui/`** = Boss-Werkstatt am API-Port), typische Ports und Startbefehle. **Backend vs. Client-IOTA (Handy-first):** **`docs/BACKEND-VS-DIREKT-IOTA-ERKLAERUNG.md`**, **`docs/ARCHITECTURE-HANDY-FIRST-CLIENT-IOTA.md`**, Fahrplan **§ H.15**. **ATAK/CoT (Backlog):** **`docs/ATAK-COT-INTEGRATION-ZIELBILD.md`**, Fahrplan **§ H.9**.
