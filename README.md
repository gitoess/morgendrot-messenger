...
**Provisioning-Profil (API):** **`docs/API-INITIAL-PROFILE.md`** — optionales Feld **`initialProfile`** bei **`POST /api/provision-device`**; Kontakt-Übernahme **`POST /api/contact-labels/apply-initial-profile`**. **Einsatz-Vorlagen (Boss-PC):** **`docs/API-EINSATZ-ROLE-TEMPLATES.md`** (`GET/POST /api/einsatz-role-templates`).
