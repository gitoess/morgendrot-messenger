...
**Mailbox & Kontakte (Kurz):** Begriffe Shared/Private/Team — **`docs/MAILBOX-BEGRIFFE-UND-NUTZUNG.md`**, Sendewege — **`docs/SENDEWEGE-KANAL-MAILBOX-UEBERSICHT.md`**. **Gruppe + Mailbox:** pairwise an alle Mitglieder (**`frontend/frontend/lib/group-mailbox-pairwise-send.ts`**). Pro Kontakt **vier Ziel-Slots (M4e)** — **`docs/KONTAKT-MAILBOX-VIER-SLOTS-ZIELBILD.md`**. Team **1× Fee**-Broadcast: Backlog **`docs/TEAM-MAILBOX-BROADCAST-1TX-KONZEPT.md`**.
