...
2. **`docs/DEV-START.md`** — Lokales Setup, Rollen (`npm run dev:role:*`), zwei Web-Oberflächen (**`frontend/`** = Next, **`ui/`** = Boss-Werkstatt). Client-IOTA wenn aktiv: **`docs/BACKEND-VS-DIREKT-IOTA-ERKLAERUNG.md`**, **§ H.15**
...
**Zwei Next-Produkte (gleicher Port 3341, getrennte Builds):** **`npm run dev`** = **Morgendrot Projekt** (volles Dashboard). **`npm run dev:messenger`** = **Morgendrot Messenger** (Einsatz-App, eigenes Bundle ohne Lock/Monitor/Radar). Beide im **Repo-Root** starten **Backend + UI**; nur `cd frontend && npm run dev:messenger` startet **ohne** API → „Keine Verbindung zum Backend“. Details: **`docs/PRODUCT-MESSENGER-VS-PROJEKT.md`**.

**Messenger-Rolle (Einsatzleitung ein/aus):** In **`.env` im Repo-Root** — **`ROLE=messenger`** = Wanderer/Solo/Helfer (**ohne** Einsatzleitung-Tab); **`ROLE=boss`** oder **`ROLE=kommandant`** = Stab **mit** Einsatzleitung. Nach Änderung Backend neu starten (`npm run dev:stop` → `npm run dev:messenger`). Ausführlich: **`docs/DEV-START.md`** → Abschnitt *Messenger: Wanderer vs. Boss*.
...
**Starten:** **`npm start`** – Backend + Streams in einem Terminal. **`npm run dev`** – Backend + **Projekt**-Next-UI. **`npm run dev:messenger`** – Backend + **Messenger**-Next-UI (Hot Reload). Details: **`docs/START-ANLEITUNG.md`**, **`docs/DEV-START.md`**, **`docs/PRODUCT-MESSENGER-VS-PROJEKT.md`**.

**Next-Messenger (`frontend/`):** Entwicklung läuft mit **`next dev --webpack`** (siehe `frontend/package.json`), damit der **Meshtastic**-Client-Bundle einen kompatiblen **`util`**. **Sendepfad × Kanal (2026-05):** Online = alle Kanäle; **Funk** = 1:1 + **Gruppe** (Meshtastic Secondary Channel, Roadmap **§ H.3o**); **Pinnwand** nur **Online** (IOTA-Brett ≠ offenes Mesh-Broadcast). **Telegram:** Mehrfach-Chat-IDs (Ist); Boss-Gruppenalarm + Telefonbuch Multi-Pick → **Roadmap § B4**. Im Chat: **Klartext + „funk“** = Standard-Meshtastic-Text (**an Node** oder Broadcast auf Primary — **Verschlüsselung über Kanal-PSK** noch Backlog **§ H.3o**, derzeit Klartext), derzeit **ohne Sprachmemo-Anhänge**; optional **„LoRa + eigene Verankerung“** (Pfad 4, MVP): nach erfolgreichem Klartext-Funk automatisch **Klartext-Mailbox an die eigene Adresse** + optionale Forensic-Attestation — **ohne** Peer-ECDH; aktuell für **Kurztext + LoRa-Bildzweiteiler (LUMA/CHROMA)**. **S-ARQ (`MORG_SEG_V1`):** Posteingang kollabiert Segment-Zeilen zu **einer Leit-Zeile** pro Session; Anzeige über **`MorgSegV1ChatSink`** (Fortschritts-Raster, Luma-/Chroma-JPEG nach Reassembly, NAK optional über dieselbe Funk-Klartext-Route) — Spez **`docs/LORA-MORGENDROT-S-ARQ-SPEC.md`**. Entwurf für spaeteres LoRa->IOTA-Relay-Envelope: **`docs/MORG_TX_RELAY_V1-SPEC.md`**. **Import/Export (Posteingang):** enthält eine **R1 Rohdaten-Submit (Beta)**-Ansicht mit vereinfachtem Hauptpfad (**Builder -> signieren -> „Zu LoRa senden“**), automatischer Feldübernahme aus importierten Envelopes und optionalen Expertenblöcken statt doppelter Hauptaktionen; **R2 sponsored** bleibt bewusst Zukunfts-Backlog. **Verschlüsselt + „funk“** = Mesh v2 und braucht **/connect** wie bisher. **Funk + Foto:** bei **privat + verschlüsselt + „funk“** wird ein zuvor IOTA-kompaktes Bild **automatisch** per Backend in **LUMA+CHROMA** umgewandelt (`/api/compact-blob-to-lora-wires`); sonst **online** oder direkt unter Funk ein Bild wählen (dann sofort LoRa-Encode). **Routing-Fehler** der Firmware (z. B. `TOO_LARGE`, `NO_CHANNEL`) werden in der UI mit **Kurz-Tipp** erklärt; **`TIMEOUT` (3)** bei Broadcast wird nicht als harter Abbruch gewertet. **LongFast / komprimierte Klartexte (Port 7):** werden im Client entpackt, damit der Posteingang auch dann Zeilen zeigt, wenn die Meshtastic-JS-Library kein `onMessagePacket` für komprimierte Texte auslöst. **Heltec „empfängt, Browser nicht“:** zuerst **USB neu** / **Trennen & neu koppeln**; im Setup **RX:** und **Events:** prüfen; bei Bedarf **`localStorage`**-Schalter **`morgendrot.meshRxDebug`** = **`1`** für Konsolenzeilen **`[morgendrot mesh]`** (siehe Setup-Hinweis). Gesendete/empfangene **Mesh-Zeilen** optional **lokal** archiviert; Posteingang-Filter **„Nur LoRa/Mesh“** und **„Nur IOTA“** (reine Mailbox-Zeilen); **IOTA/Mailbox**-Partnerzeile nur Adressen mit mindestens einer solchen Zeile. **Posteingang:** eingehende Funkzeilen werden nicht mehr fälschlich durch **Partner-/Richtungsfilter** wie reine Mailbox-Zeilen versteckt. **Privater Chat-Kopf:** Kurztext nur für Screenreader; **Posteingang:** **Pakete**/**Import/Export**-Menüs, Partner-Chips **löschbar**; **Boss:** **Einsatz-Profil** im Posteingang, eingebettet ohne langen Fließtext. Im Panel **„IDs zum Kopieren · Direkt-RPC · Funk“** ist Heartbeat im Messenger ausgeblendet; **Handshake -> Peer-Pub Auto-Fill** übernimmt ECDH-Peer-Pubs automatisch, während manuelle Key-/LoRa-Power-Eingriffe nur in Expertenoptionen erscheinen (im Messenger standardmäßig versteckt; Flag `morgendrot.dev.expertTools=1`). **`docs/MONOREPO-NEXT-AND-SHARED.md`** (Turbopack vs. Webpack).
...
**Mailbox & Kontakte (Kurz):** Begriffe Shared/Private/Team — **`docs/MAILBOX-BEGRIFFE-UND-NUTZUNG.md`**, Sendewege — **`docs/SENDEWEGE-KANAL-MAILBOX-UEBERSICHT.md`**. **Gruppe + Mailbox:** pairwise an alle Mitglieder (**`frontend/frontend/lib/group-mailbox-pairwise-send.ts`**). **Meshtastic-Verschlüsselung (Channel-PSK, Secondary Channels, Steuerung App vs. Messenger):** **Roadmap § H.3o**. **Telegram Boss-Gruppenalarm / Telefonbuch Multi-Pick:** **Roadmap § B4**. Pro Kontakt **vier Ziel-Slots (M4e)** — **`docs/KONTAKT-MAILBOX-VIER-SLOTS-ZIELBILD.md`**. Team **1× Fee**-Broadcast: Backlog **`docs/TEAM-MAILBOX-BROADCAST-1TX-KONZEPT.md`**.
...
| `npm run dev` | **Backend + Morgendrot Projekt** (Next): API **3342** + UI **3341**. Browser: http://127.0.0.1:3341/ — Lite-UI: http://127.0.0.1:3342/ |
| `npm run dev:messenger` | Wie `dev`, aber Next lädt **`messenger-dashboard`** (`NEXT_PUBLIC_MORG_PRODUCT=messenger`) — Einsatz-Messenger mit Hot Reload |
| `npm run dev:lan` / `dev:messenger:lan` | Wie `dev` bzw. `dev:messenger`, API + Next auf **0.0.0.0** (Handy im WLAN) |
| `npm run dev:frontend` / `dev:frontend:messenger` | **Nur** Next (3341); Backend separat mit `npm run start:secrets` — sonst Offline-Hinweis in der UI |
...
**Checkliste (empfohlen):** **`docs/DEPLOY-CHECKLIST.md`** — Deploy → `create_globals` → `.env` → Neustart → **`package-profiles.manifest.json`** → Test.
...
Publizieren mit `iota client publish ...` oder im Root **`npm run deploy:move-package`**. Nach Deploy: **PACKAGE_ID** in `.env` eintragen — Details siehe Checkliste oben.
