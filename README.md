...
**Next-Messenger (`frontend/`):** Entwicklung läuft mit **`next dev --webpack`** (siehe `frontend/package.json`), damit der **Meshtastic**-Client-Bundle einen kompatiblen **`util`**. Im Chat: **Klartext + „funk“** = Standard-Meshtastic-Text (**Broadcast** oder optional **Node-ID** `!…`); **verschlüsselt + „funk“** = Mesh v2 und braucht **/connect** wie bisher. **`docs/MONOREPO-NEXT-AND-SHARED.md`** (Turbopack vs. Webpack).
