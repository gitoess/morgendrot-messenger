...
- **§ H.15 — Live-Send (Composer) hybrid:** **`use-chat-view-handle-send.ts`** — Klartext- und verschlüsselter **Online**-Mailbox-Versand: **Direct-IOTA zuerst** (`trySubmitPlaintextMailboxViaDirectIota` / `trySubmitEncryptedMailboxViaDirectIotaFromPlaintext`), bei Fehler oder fehlender Konfiguration **`sendMessage`** / **`sendEncryptedMessageWithTimeout`**. Hilfen **`canTryLivePlaintextDirectMailbox`**, **`canTryLiveEncryptedDirectMailbox`** — **`direct-iota-plain-submit.ts`**, **`direct-iota-encrypted-submit.ts`**. **Chat-Kopf:** Badge **Direkt-RPC** / **Relay** (`getDirectIotaPathUiState`), Aktualisierung per **`notifyDirectIotaUiChanged`** / **`DIRECT_IOTA_UI_CHANGED`** — **`chat-view-chat-header.tsx`**.
